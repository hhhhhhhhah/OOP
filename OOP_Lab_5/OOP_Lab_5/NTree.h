...
public:
    ...
    bool empty();
    Iterator<NTreeNode<T>, T> begin();
    Iterator<NTreeNode<T>, T> end();
    virtual ~NTree();
private:
...
template<class T>
Iterator<NTreeNode<T>, T> NTree<T>::begin() {
    if (root)
        return Iterator<NTreeNode<T>, T>(root->Begin());
    else
        return Iterator<NTreeNode<T>, T>(nullptr);
}

template<class T>
Iterator<NTreeNode<T>, T> NTree<T>::end() {
    return Iterator<NTreeNode<T>, T>(nullptr);
}
